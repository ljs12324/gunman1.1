...
def detction():
    ...
    while out:
        ...
        if (float(resluttime[0]) < 0.01 or cycles_nums == 1) and zhishu == 1:
            ...
            if zengjiashijian > maxzengjiashijain:
                maxzengjiashijain = zengjiashijian
            print(Period_Time)
            time.sleep(Period_Time)
            cycles_nums = cycles_nums + 1
            ...
            if zengjiashijian == 0.125:
                ...
                resluttime = t1.get_result()
                print(resluttime)
                if float(resluttime[0]) < 0.01 :
                    pass
                else:
                    Period_Time = Period_Time - 0.125
                time.sleep(Period_Time + 2)
    ...
    t2 = MyThread(generate_flow, args=())
    t2.start()
    resluttime1 = t1.get_result()
    print(resluttime1)
    if (float(resluttime1[0]) > 0.01 and float(resluttime[0]) < 0.01) or (float(resluttime1[0]) < 0.01 and float(resluttime[0]) > 0.01):
        print("is not idel_timeout!!!!")
        shuimiantime = Period_Time
        time.sleep(Period_Time*3)
        out = True
        detashijian = maxzengjiashijain
        print("tetst is maxyime:",detashijian)
        chaoshi = 1.0
        while out:
            Pre_Period_Time = Period_Time
            Period_Time = Period_Time + detashijian
            if Period_Time == chaoshi:
                detashijian = detashijian/2
                Period_Time = Pre_Period_Time + detashijian
            t1 = MyThread(job2, args=(timelist,))
            t1.start()
            t2 = MyThread(generate_flow, args=())
            t2.start()
            ...
            t2 = MyThread(generate_flow, args=())
            t2.start()
            resluttime2 = t1.get_result()
            print(resluttime2)
            print(detashijian)
            if float(resluttime2[0]) < 0.01 and detashijian == 0.125:
                print("zhaodaol")
                print("hand_timeout:", Period_Time)
                out = False
            if float(resluttime2[0]) < 0.01 and out == True:
                print("meichaoshi")
                print(Period_Time)
                print("shuimianshijian:", shuimiantime)
                time.sleep(shuimiantime * 3)
                pass
            elif out == True:
                print("chaoshil")
                print(Period_Time)
                ...
                print("shuimianshijian:",shuimiantime)
                time.sleep(shuimiantime * 3)
                if detashijian == 0.125:
                    Period_Time = Period_Time - 0.125
                    out = False
                    print("hand_timeout:", Period_Time)
                else:
                    detashijian = detashijian / 2
                    Period_Time = Pre_Period_Time
    elif float(resluttime1[0]) > 0.01 and float(resluttime[0]) > 0.01:
        havetime = 0
        print("is idel_timeout!!!!")
        Period_Time = int(Period_Time)
        Period_Time = Period_Time + 0.5
        print("shuimianshijian",Period_Time+3)
        time.sleep(Period_Time+3)
        t1 = MyThread(job2, args=(timelist,))
        t1.start()
        t2 = MyThread(generate_flow, args=())
        t2.start()
        resluttime3 = t1.get_result()
        print(resluttime3)
        cishu = 11
        jishu = 0
        while cishu:
            print(Period_Time)
            time.sleep(Period_Time-1)
            t1 = MyThread(job2, args=(timelist,))
            t1.start()
            t2 = MyThread(generate_flow, args=())
            t2.start()
            resluttime3 = t1.get_result()
            print(resluttime3)
            if float(resluttime3[0]) < 0.01:
                jishu += 1
                print("xunhuancishu:", jishu)
                cishu = cishu - 1
                if cishu == 0:
                    havetime = 0
                    print("no hand_timeout!!!!")
            else:
                cishu = 0
                havetime = 1
                print("have hand_timeout!!!")
        if havetime == 1:
            print("start hand_timeout!!!!")
            print("shuimianshijian:", Period_Time+3)
            time.sleep(Period_Time+3)
            print("really start dection hand-timeout!!!!")
            kaishi = True
            zuizhongshijian = Period_Time / 2
            erfenfa = zuizhongshijian
            print("zuizhongshijian:", zuizhongshijian)
            while kaishi:
                time.sleep(Period_Time + 3)
                # while jishu:
                #     t1 = MyThread(job2, args=(timelist,))
                ...
                #     resluttime3 = t1.get_result()
                #     print(resluttime3)
                #     print("xunhuanshijian:", Period_Time)
                #     time.sleep(Period_Time)
                #     jishu = jishu - 1
                ...
                resluttime3 = t1.get_result()
                print(resluttime3)
                time.sleep(zuizhongshijian)
                print("shuimianzengjiadeshijian:", zuizhongshijian)
                t1 = MyThread(job2, args=(timelist,))
                t1.start()
                t2 = MyThread(generate_flow, args=())
                t2.start()
                ...
                if float(resluttime3[0]) < 0.01:
                    erfenfa = erfenfa / 2
                    print("daoqianweizhi", erfenfa)
                    zuizhongshijian = zuizhongshijian + erfenfa
                    print("zengjiahou de shijian :", zuizhongshijian)
                else:
                    erfenfa = erfenfa / 2
                    print("daoqianweizhi", erfenfa)
                    zuizhongshijian = zuizhongshijian - erfenfa
                    print("jianshaode shijian :", zuizhongshijian)
